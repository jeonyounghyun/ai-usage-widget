...
3. 검은 창의 한글 안내를 따라갑니다. 직접 할 일은 세 가지뿐입니다.
   - **Claude 로그인**: 브라우저가 열리면 Claude 계정으로 로그인 → Authorize 클릭 → 검은 화면에 완료 문구가 뜨면 끝
...
| 캐릭터 | 사용률이 빨리 오를수록 빨리 걷고(소진 속도), 10분간 변화 없으면 서서 쉬고, 30분이면 잠듦. 100%면 드러눕고, 리셋되면 기지개 |
| 도넛 안쪽 얇은 링 | 창(5시간·7일)의 시간이 얼마나 지났는지. 사용률 링보다 앞서 있으면 여유, 뒤처져 있으면 빠듯 |
| 미니 모드 색 점 | 이름 옆 점 = 5시간·7일 중 나쁜 쪽 색 |
...
- **캐릭터 스타일**: 부드러운 고양이(기본) / 검은 고양이 / 삼색 고양이 / 강아지 / 토끼 / 젤리 / 도트 고양이 / 없음
- **Claude 표시 / GPT 표시**: 한쪽만 남길 수 있음 (카드가 절반 폭으로)
- **작업표시줄 미니 모드**
- **투명도**: 100 / 85 / 70 / 55%
- **테마**: 자동(Windows 다크 모드 따라감) / 밝게 / 어둡게
- **마우스를 올릴 때만 선명하게**: 평소엔 흐리게(45%) 두고 마우스를 올리면 설정한 투명도로
...
| 상황 | 해결 |
|---|---|
| 위젯에 "재로그인 필요" | 우클릭 → 문제 해결 → **Claude 다시 로그인** (브라우저 로그인 → Authorize → 완료 문구 확인) |
...
| Claude 로그인 방식을 물으면 | **"Claude account with subscription"**(구독 계정). 다른 방식은 한도가 없어 숫자가 안 뜸 |
